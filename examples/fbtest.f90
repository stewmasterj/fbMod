...
! DATE: August 2018
! compile: gfortran fbMod.f90 fbtest.f90 -o fbtest
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!80
program fbtest
use fbMod
implicit none
...
call fb_pixel(1,1,px) !record 1
px=char(255)//char(0)//char(0)//char(0)
call fb_pixel(2,1,px) !record 2
px=char(0)//char(255)//char(0)//char(0)
call fb_pixel(3,1,px) !record 3
px=char(0)//char(0)//char(255)//char(0)
call fb_pixel(4,1,px) !record 4
call fb_pixel(fb%width,1,px) !top right
call fb_pixel(fb%line+1,1,px) ! second row top left?
...
call fb_line2c(199,399, 199,699, px, bpx)
...
call fb_filltriangle3c( 700,800, 750,750, 780,830, red, green, blue )
call fb_filltriangle3c( 800,800, 850,750, 880,830, cyan, magenta, yellow )
call fb_filltriangle3c( 600,800, 650,750, 680,830, px, white, black )
...
! turn on Z-buffer
fb%Lzbuff = .true.
allocate( fb%zbuff(fb%width, fb%height) )
fb%zbuff = -1.0 !clear Z-Buffer as well (-1 is INF))
! test overlaps
call fb_filltriangle3c( 1000,600, 1000,800, 1100,700, white, white, blue, sr, 1.0, 1.0, 0.0 )
call fb_filltriangle3c( 1000,600, 1000,800, 1200,700, white, white, red, sr, 1.0, 1.0, 1.0 )
! the second triangle will obscure the first, if no zbuffer check
call fb_line(1000,700, 1200,700, px)
call fb_line(1100,600, 1100,800, px)
