...
! date: April 28, 2019
! compile: gfortran ../fbMod2.f90 heatdiff.f90 -o heatdiff
! run: ./heatdiff
program heatplot
use fbMod
implicit none
...
call fb%loadScreen !grab what's on the screen into the buffer
...
call fb%matrixplot( dat, ar, sr, "bw  " )
call fb%display
...
do t = 1, Nsteps
 ...
 if (mod(t,500).eq.0) then
  ...
  call fb%matrixplot( dat, ar, sr, "bw  " )
  write(chr,'(i8)') t 
  call fb%putString( chr, 300, 180, 1, char(255)//char(255)//char(255)//char(0), char(0)//char(0)//char(0)//char(0) )
  call fb%display
 endif
enddo

call fb%save("heatdiff.ppm",2)
